...
def place_artifact_to_cal_area(nozzle, part, target_center_loc, config, angle_deg=0.0, safe_move=True, logger=None):
    ...
    try:
        ...
        log_msg(logger, "After explicit XY correction readback: %s" % loc_to_log(before_lower))
...
def solve_head_offset(config, nozzle=None, camera=None, machine_obj=None, part=None, logger=None):
    ...
    try:
        ...
        pick_artifact_from_storage(nozzle, part, storage_found["center_location"], storage_transfer_config,
        ...
        current = find_artifact_top(camera, nozzle, part, calibration_pose, config, logger=logger)
        old_center = current["center_location"]
        log_msg(logger, "Old center: %s" % loc_to_dict(old_center))
        ...
        for angle in angles:
            log_msg(logger, "Pairwise angle start: %.4f" % angle)
            old_center = old_center.convertToUnits(LengthUnit.Millimeters)
            pick_angle = float(angle)
            place_angle = unwrapped_plus_180(pick_angle, config.get("pairwise_180_direction", 1.0))
            pick_result = pick_artifact_from_storage(nozzle, part, old_center, {
                "storage_location": config["calibration_location"],
                "part_height_mm": config["part_height_mm"],
                "vacuum_ms": config.get("vacuum_ms", 150),
                "post_pick_lift_mm": config.get("post_pick_lift_mm", 2.0),
            ...
            step_mag = math.sqrt(dx * dx + dy * dy)
            if step_mag > max_single_delta:
                max_single_delta = step_mag
            sum_delta_mag_sq += step_mag * step_mag
            sum_dx += dx
            sum_dy += dy
            log_msg(logger, "New center: %s" % loc_to_dict(new_center))
            log_msg(logger, "Per-angle delta: angle %.4f X %.6f Y %.6f mag %.6f" % (
                angle, dx, dy, step_mag))
            if "pairwise_max_step_center_walk_mm" in config:
                max_step_walk = float(config["pairwise_max_step_center_walk_mm"])
                if step_mag > max_step_walk:
                    raise Exception("Pairwise center walk %.6f exceeds %.6f at angle %.4f" % (
                        step_mag, max_step_walk, angle))

            per_angle_records.append({
                "angle_deg": angle,
            ...
            log_msg(logger, "Pairwise angle end: %.4f" % angle)

        count = float(len(angles))
        pairwise_rms_delta = math.sqrt(sum_delta_mag_sq / count)
        sign = float(config.get("head_offset_sign", 1.0))
        delta_x = sign * sum_dx / (2.0 * count)
        delta_y = sign * sum_dy / (2.0 * count)
        correction_mag = math.sqrt(delta_x * delta_x + delta_y * delta_y)
        log_msg(logger, "Pairwise quality: max single delta %.6f rms delta %.6f final correction %.6f" % (
            max_single_delta, pairwise_rms_delta, correction_mag))
        if "pairwise_max_single_delta_mm" in config:
            max_single = float(config["pairwise_max_single_delta_mm"])
            if max_single_delta > max_single:
                raise Exception("Pairwise max single delta %.6f exceeds %.6f" % (
                    max_single_delta, max_single))
        if "pairwise_max_rms_delta_mm" in config:
            max_rms = float(config["pairwise_max_rms_delta_mm"])
            if pairwise_rms_delta > max_rms:
                raise Exception("Pairwise RMS delta %.6f exceeds %.6f" % (
                    pairwise_rms_delta, max_rms))
        ...
        return {
            ...
            "pairwise_max_single_delta_mm": max_single_delta,
            "pairwise_rms_delta_mm": pairwise_rms_delta,
            "final_correction_mag_mm": correction_mag,
            "angles_deg": angles,
            "per_angle_records": per_angle_records,
            "final_artifact_pose": loc_to_dict(old_center),
        }
