...
def verify_square_tip_at_command_zero_only(config, nozzle=None, camera=None, machine_obj=None, logger=None, label=""):
    machine_obj = get_machine(machine_obj)
    nozzle = get_nozzle(machine_obj, nozzle)
    camera = get_bottom_camera(machine_obj, camera)
    desired_zero = float(config.get("bottom_square_zero_deg", 0.0))
    ...
    if str(label) == "post_builtin":
        prefix = "Post-built-in square verify"
    elif str(label) == "pre_builtin_after_head_offset":
        prefix = "Pre-built-in square verify"
    else:
        prefix = "Square verify"

    log_msg(logger, "%s start" % prefix)
    log_msg(logger, "%s target: %s" % (prefix, loc_to_log(target)))
    move_direct(nozzle, target)
    read_loc = nozzle.getLocation().convertToUnits(LengthUnit.Millimeters)
    log_msg(logger, "%s actual nozzle: %s" % (prefix, loc_to_log(read_loc)))

    raw_angle, error, sample = read_center_square_error(
        config, nozzle, camera, machine_obj, target, desired_zero, logger=logger)
    read_loc = nozzle.getLocation().convertToUnits(LengthUnit.Millimeters)
    threshold = None
    if "post_builtin_square_verify_max_error_deg" in config:
        threshold = float(config["post_builtin_square_verify_max_error_deg"])

    log_msg(logger, "%s raw angle: %.6f" % (prefix, float(raw_angle)))
    log_msg(logger, "%s normalized error: %.6f" % (prefix, float(error)))

    ok = True
    if threshold is not None and abs(float(error)) > threshold:
        ok = False
        msg = "%s normalized error %.6f exceeds %.6f" % (
            prefix, float(error), threshold)
        if str(label) == "post_builtin" and bool_config(config, "post_builtin_square_verify_fail_on_error", False):
            raise Exception(msg)
        log_msg(logger, "Warning: %s" % msg)
    ...
    return {
        ...
        "raw_angle_deg": raw_angle,
        "normalized_error_deg": error,
        "nozzle_location_mm": loc_to_dict(read_loc),
        "desired_zero_deg": desired_zero,
        "max_allowed_error_deg": threshold,
    }


def resquare_and_true_zero_after_pairwise(config, nozzle=None, camera=None, machine_obj=None, logger=None):
    machine_obj = get_machine(machine_obj)
    nozzle = get_nozzle(machine_obj, nozzle)
    camera = get_bottom_camera(machine_obj, camera)
    desired_zero = float(config.get("bottom_square_zero_deg", 0.0))
    target_error = float(config.get(
        "post_pairwise_resquare_target_error_deg",
        config.get("square_refine_target_error_deg", 0.75)))
    max_iterations = int(config.get("post_pairwise_resquare_max_iterations", 3))
    fail_on_error = bool_config(config, "post_pairwise_resquare_fail_on_error", True)
    ...
    result = {
        ...
        "final_raw_angle_deg": None,
        "final_nozzle_rotation_deg": None,
        "true_zero_result": None,
        "samples": [],
        "records": [],
    }

    log_msg(logger, "Post-pairwise resquare start")
    base_loc = bottom_calibration_location(nozzle, camera)
    target = mm_loc(
        base_loc.getX(),
        base_loc.getY(),
    ...
    raw_angle, error, sample = read_center_square_error(
        config, nozzle, camera, machine_obj, target, desired_zero, logger=logger)
    result["initial_error_deg"] = error
    result["final_error_deg"] = error
    result["final_raw_angle_deg"] = raw_angle
    result["samples"].append(sample)
    result["records"].append({
        "phase": "initial",
        "raw_angle_deg": raw_angle,
        "normalized_error_deg": error,
        "target_location_mm": loc_to_dict(target),
        "sample": sample,
    })
    log_msg(logger, "Post-pairwise resquare initial raw angle %.6f" % float(raw_angle))
    log_msg(logger, "Post-pairwise resquare initial normalized error %.6f" % float(error))

    if abs(float(error)) > target_error:
        result["attempted"] = True
        for i in range(max_iterations):
            current = nozzle.getLocation().convertToUnits(LengthUnit.Millimeters)
            current_r = float(current.getRotation())
            new_r = normalize_rotation(current_r + square_angle_sign * float(error))
            log_msg(logger, "Post-pairwise resquare correction iteration %d current R %.6f error %.6f new R %.6f" % (
                i + 1, current_r, float(error), new_r))
            target = mm_loc(current.getX(), current.getY(), current.getZ(), new_r)
            move_direct(nozzle, target)
            raw_angle, error, sample = read_center_square_error(
                config, nozzle, camera, machine_obj, target, desired_zero, logger=logger)
    ...
    if abs(float(result["final_error_deg"])) <= target_error:
        if result["attempted"]:
            log_msg(logger, "Post-pairwise resquare true-zero start")
            true_zero_result = true_zero_nozzle_rotation_at_current_position(nozzle, config, logger=logger)
            result["true_zero_result"] = true_zero_result
            current = nozzle.getLocation().convertToUnits(LengthUnit.Millimeters)
            zero_target = mm_loc(
                current.getX(),
                current.getY(),
            ...
            raw_angle, error, sample = read_center_square_error(
                config, nozzle, camera, machine_obj, zero_target, desired_zero, logger=logger)
            result["final_error_deg"] = error
            result["final_raw_angle_deg"] = raw_angle
            result["samples"].append(sample)
            result["records"].append({
                ...
                "raw_angle_deg": raw_angle,
                "normalized_error_deg": error,
                "target_location_mm": loc_to_dict(zero_target),
                "sample": sample,
            })

        final_loc = nozzle.getLocation().convertToUnits(LengthUnit.Millimeters)
        result["final_nozzle_rotation_deg"] = float(final_loc.getRotation())
        result["ok"] = abs(float(result["final_error_deg"])) <= target_error
    else:
        final_loc = nozzle.getLocation().convertToUnits(LengthUnit.Millimeters)
        result["final_nozzle_rotation_deg"] = float(final_loc.getRotation())

    log_msg(logger, "Post-pairwise resquare final verify raw angle %.6f" % (
        float(result["final_raw_angle_deg"])))
    log_msg(logger, "Post-pairwise resquare final verify normalized error %.6f" % (
        float(result["final_error_deg"])))
    log_msg(logger, "Post-pairwise resquare complete ok %s" % result["ok"])

    if not result["ok"]:
        msg = "Post-pairwise resquare final error %.6f exceeds %.6f" % (
            float(result["final_error_deg"]), target_error)
        if fail_on_error:
            raise Exception(msg)
        log_msg(logger, "Warning: %s" % msg)
...
def return_artifact_to_storage(config, nozzle, camera, machine_obj, logger=None):
    ...
    found_pick_target = detection_pick_location(found)
    ...
    log_msg(logger, "Using top vision nozzle-pick target for pick: %s" % loc_to_log(found_pick_target))
    log_msg(logger, "Using top vision camera-only center for measurement/math: %s" % loc_to_log(
        found["center_location"]))
    pick = pick_artifact_from_storage(
        nozzle, part, found_pick_target, pick_config, pick_rotation_deg,
        logger=logger)
    ...
    return {
        ...
        "measurement": {
            ...
            "nozzle_pick_location": loc_to_dict(found_pick_target),
...
def run_full_sequence(config, nozzle=None, machine_obj=None, logger=None):
    ...
    result = {
        ...
        "post_pairwise_resquare": None,
        "pre_builtin_square_verify": None,
        "post_builtin_square_verify": None,
    ...
    try:
        ...
        if bool_config(config, "post_pairwise_resquare_enabled", True):
            result["post_pairwise_resquare"] = resquare_and_true_zero_after_pairwise(
                config,
                nozzle=nozzle,
                camera=None,
        ...
        if bool_config(config, "pre_builtin_square_verify_enabled", True):
            result["pre_builtin_square_verify"] = verify_square_tip_at_command_zero_only(
                config,
                nozzle=nozzle,
                camera=None,
                machine_obj=machine_obj,
                logger=logger,
                label="pre_builtin_after_head_offset"
            )
        ...
        try:
            ...
            if bool_config(config, "post_builtin_square_verify_enabled", True):
                result["post_builtin_square_verify"] = verify_square_tip_at_command_zero_only(
                    config,
                    nozzle=nozzle,
                    camera=None,
                    machine_obj=machine_obj,
